function [accur, prec, rec, spec, npv, fm]=classifier(excerpts,bin_scors, settings)
...
for i=1:num_of_exc
    %single channel
    %[cur_labels, cur_feat]=const_segmentation(excerpts.(names_exc{i}), bin_scors.(names_bin{i}),channel,settings);
    %multi channel
    [cur_labels, cur_feat]=const_segmentation_multichannel(excerpts.(names_exc{i}), bin_scors.(names_bin{i}), settings);
